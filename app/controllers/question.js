...
var Alternative = require('../models/alternative.js')(
  db.sequelize,
  db.Sequelize,
)
...
exports.getQuestionsWithPagination = async (req, res) => {
  ...
  if (!examId)
    return res
      .status(400)
      .json({ success: false, error: 'Simulado não informado!' })
  else {
    //Fetch exam
    Participation.findOne({ where: { practise_exam_id: examId } })
      .then(async participation => {
        if (!participation) {
          return res
            .status(401)
            .json({ success: false, error: 'Simulado não informado!' })
        } else if (lastQuestion >= participation.numberOfQuestions)
          return res
            .status(400)
            .json({ success: false, error: 'No more questions for this exam' })
        else {
          ...
          let questions = await db.sequelize
            ...
            .then(questions =>
              db.sequelize.Promise.each(
                questions,
                async question =>
                  (question.alternatives = await Alternative.findAll({
          ...
          let questionsTotal = await db.sequelize.query(queryCount, {
          ...
          if (questions.length)
            ...
            return res.status(401).json({
              success: false,
              error: 'As questões deste simulado não foram encontradas.',
            })
        }
      })
      .catch(() =>
        res.status(401).json({
          success: false,
          error: 'Ocorreu um erro ao buscar as questões.',
        }),
      )
  }
}
...
const validate = body => {
  ...
  for (let p in properties) {
    if (!body.hasOwnProperty(p) || body[p] === '') return false
  }
  return true
}
  ...
  try {
    ...
    if (!valid) {
      return res.status(400).json({
        success: false,
        error: 'Dados informados invalidados ou faltando',
      })
    } else {
...
const isCoordinator = async user =>
  !!(await Coordinator.findOne({ where: { user_id: user.id } }))
...
exports.approve = async (req, res) => {
  try {
    ...
    if (!isUserCoordinator) {
      return res
        .status(400)
        .json({ success: false, error: 'Usuário sem permissão para esta ação' })
    } else {
      await Question.update(
        {
    ...
    return res.status(400).json({
      success: false,
      error: 'Ocorreu um erro ao aprovar esta questão',
    })
  }
}
...
exports.create = async (req, res) => {
  try {
    ...
    const resData = await Question.create({
      ...question,
      professor_id: 1,
      coordinator_id: 1,
      subarea_id: 1,
    ...
    const { alternativeA, alternativeB, alternativeC, alternativeD } = question
    const alternatives = [
      [alternativeA, 'A'],
      [alternativeB, 'B'],
      [alternativeC, 'C'],
      ...
      professor_id: resData.professor_id,
      correct: letter === resData.right_alternative,
      letter,
      description,
    }))

    const altRes = await Alternative.bulkCreate(alternatives)
    console.log(altRes)
    res.status(200).json({
      success: true,
      message: 'Questão atualizada',
      question: resData.toJSON(),
      alternatives: altRes.map(v => v.toJSON())
    })
  } catch (e) {
    console.log(e)
    return res
      .status(400)
      .json({ success: false, error: 'Ocorreu um erro ao criar a questão' })
  }
}
